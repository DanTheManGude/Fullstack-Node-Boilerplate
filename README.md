...
This application is a boilerplate for a Node Express [backend](#backend) and React [frontend](#frontend).
Production url: http://appname.domain
...
- .circleci/ –– Configuration for running [CircleCI](https://circleci.com/gh/GitHubUsername/App-Name "CircleCI for the application")
...
Be sure to follow these guidelines for making a [PR](https://github.com/GitHubUsername/App-Name/pulls "CircleCI for the application").
...
- `master` source for PROD deploys <https://appname.recipes>
- `qa` Staging QA area before rolling something out to PROD. <https://qa.appname.recipes>
- `dev` has a live DEV deploy to test changes <https://dev.appname.recipes>
...
Testing is powered by Chai and Mocha. To run the test use `npm test`. Tests are defined in `/src/app.test.js` and test the methods in `/src/app.js`. [CircleCI](https://circleci.com/gh/GitHubUsername/App-Name "CircleCI for the application") runs the tests automatically on every push. Be sure all tests pass especially when opening a PR.

## Heroku Deployment

([Return to Table of Contents](#Table-of-Contents))\
[Heroku](http://heroku.com "Heroku Homepage") is used to for continuous deployment. Any open PRs coming from within the repo (ie not outside people), Heroku will make a temporary app to deploy it. The url that it can be found at is <https://app-name-pr-#.herokuapp.com/> where the # is replaced with your PR number.\
